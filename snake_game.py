# import relevent modules and libraries
import turtle
import random
import time
import math
...
# snake start location coordinates --> (0,0) is center of screen
SNAKE_START_LOC_H = 0 # snake starts location (height or y)
SNAKE_START_LOC_W = 0 # snake starts location (width or x)
...
class Snake(gym.Env):
    def __init__(self, human=False, env_info={'state_space':None}):
        ...
        ## GAME CREATION WITH PYTHON TURTLE 
        # screen/background
        ...
        self.snake.goto(SNAKE_START_LOC_H, SNAKE_START_LOC_W)
    ...
    # create a random location for the apple to spawn after its eaten
    ...
    # update score when apple is eaten 
    ...
    # when dead, clear score and reset 
    def reset_score(self):
        self.score.clear()
        self.total = 0
        self.score.write(f"Total: {self.total}   Highest: {self.maximum}", align='center', font=('Courier', 18, 'normal'))
                    
    # function to add apple to body of snake
    ...
    # move body of snake in the direction of the head and change by 1 unit after arrow key is pressed
    ...
    # measure distance between snake and apple
    def measure_distance(self):
        self.prev_dist = self.dist
        self.dist = math.sqrt((self.snake.xcor()-self.apple.xcor())**2 + (self.snake.ycor()-self.apple.ycor())**2)

    # If snake hits itself, reset score because dead
    ...
    # If snake hits wall, reset score because dead 
    def wall_check(self):
    ...
    # function to step through the game
    ...
    def get_state(self):
        ...
        # these if statements are for the different state spaces
        # A state is a discrete space representing the set of all possible configurations of a "system"
